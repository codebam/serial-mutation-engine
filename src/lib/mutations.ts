import { TOK_VARINT, TOK_VARBIT, type Serial, type Block, type State } from './types';
...
function createRandomRepeatingPart(
	minPartSize: number,
	maxPartSize: number,
	assetPool: number[]
): number[] {
...
export const scrambleAndAppendFromRepo: Mutation = (serial, state) => {
	...
	if (repository.length > 0) {
		...
		if (lastQuarter.length > 0) {
			...
			const maxNumberOfAppends = Math.floor(
				((targetOffset || 0) * bitsPerCharacter) / bitsPerAsset
			);
...
export const repeatHighValuePart: Mutation = (serial, state) => {
	...
	const repeatingAssets: { asset: number; count: number }[] = [];
	...
	if (repeatingAssets.length === 0) {
		...
		const bitsPerCharacter = 8;
		const bitsPerAsset = 8;
		const maxNumberOfRepeats = Math.floor(
			((state!.rules.targetOffset || 0) * bitsPerCharacter) / bitsPerAsset
		);
...
export const repeatSelectedAssetMutation: Mutation = (serial, state) => {
	...
	const assetList = blocksToValues(serial);
	const newAssets = [...assetList];
	const indices = newAssets
		.map((a, i) => (a === state.selectedAsset ? i : -1))
		.filter((i) => i !== -1);
...
export function mergeSerials(
	...
	const backpackIndex = lines.findIndex((line) => line.trim().startsWith('backpack:'));
	...
	if (backpackIndex === -1) {
		...
		const newStructure = [
			...
			'    items:',
			'      backpack:',
			...serialsToInsert.flatMap((s, i) => [`        slot_${i}:`, `          serial: '${s}'`])
		];
